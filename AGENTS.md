...
- **`src/`** — plugin source:
  ...
  - `plugin-settings.ts` / `plugin-settings-component.ts` / `plugin-settings-tab.ts` — `PluginSettings` (data shape: `initialExpandLevel`, `isFullKeyDisplayEnabled`), its `PluginSettingsComponentBase` subclass, and the declarative `PluginSettingsTabBase` tab. The tab exposes **only** `initialExpandLevel`: `isFullKeyDisplayEnabled` is deliberately absent from it, being controlled by the command and the Properties-header button, with the settings component only persisting that state. (Until `initialExpandLevel` there was intentionally no tab at all; the tab exists because that setting has no other control surface.)
