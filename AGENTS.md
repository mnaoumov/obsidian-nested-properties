...
- **`src/`** — plugin source:
  ...
  - `nested-properties-note-settings.ts` — the per-note override, read from a note's `nestedProperties` frontmatter key (camelCase, mirroring the `PluginSettings` field names one-to-one, as the fleet's plugin-scoped frontmatter keys are). Read **synchronously** off `app.metadataCache.getCache(sourcePath)` rather than via `obsidian-dev-utils`' async `getFrontmatterSafe`, because the property widget's render path is synchronous. A value that is not a non-negative integer is not an override at all and falls through to the plugin setting.
