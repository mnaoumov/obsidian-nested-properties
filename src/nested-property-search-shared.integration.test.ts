 ...
 * matching this plugin's other behavioral integration tests. Android registration is DEFERRED because no
 * Android emulator / Appium server is provisioned in the current environment, so an
 * `…android.integration.test.ts` entry could not be verified green (G97's "record the specific reason"
 * escape hatch). The body is platform-agnostic (native search and nested frontmatter behave identically on
 * mobile), so enabling Android later is a one-line `registerNestedPropertySearchSuite('Android')` from an
 * android entry point once the emulator is available. The `*-shared.integration.test.ts` name keeps it out
 * of unit collection + coverage while matching no `*.desktop`/`*.android` project glob, so it runs only when
 * imported.
 */
...
/**
 * Registers the native nested-property search integration test for the given platform.
 *
